...
const ui = {
    ...
    // [*] Sửa đổi: Bỏ đi việc render biểu đồ, chỉ cập nhật daily goal
    displayHealthKpiTable: (extractedData, goals, updateDailyGoalCallback) => {
        ...
        if (!container) { 
            // Vẫn chạy các hàm khác nếu bảng không tồn tại
    ...
    // ... (Các hàm render bảng khác giữ nguyên: displayEmployeeRevenueReport, renderRevenueTableForDepartment, displayEmployeeIncomeReport, v.v...)
    // ... (Tất cả các hàm từ displayEmployeeRevenueReport đến hết giữ nguyên không đổi)
    ...
    // [*] Sửa đổi: Cập nhật hàm để dùng doanhThuChuaXuat
    renderRealtimeKpiCards: (data, goals) => {
        ...
        document.getElementById('rt-kpi-dt-thuc-sub1').innerHTML = `% HT: <span class="kpi-percentage">${ui.formatPercentage(phanTramThucHienThuc)}</span> / Target: ${ui.formatNumberOrDash(targetDTThuc / 1000000, 2)}`;
        document.getElementById('rt-kpi-dt-thuc-sub2').textContent = `DT Chưa xuất: ${ui.formatNumberOrDash(data.doanhThuChuaXuat / 1000000, 2)}`;
    
        document.getElementById('rt-kpi-dt-qd-main').textContent = ui.formatNumberOrDash(thucHienDTQD / 1000000, 2);
        document.getElementById('rt-kpi-dt-qd-sub1').innerHTML = `% HT: <span class="kpi-percentage">${ui.formatPercentage(phanTramThucHienQD)}</span> / Target: ${ui.formatNumberOrDash(targetDTQD / 1000000, 2)}`;
        document.getElementById('rt-kpi-dt-qd-sub2').textContent = `DTQĐ Chưa xuất: ${ui.formatNumberOrDash(data.doanhThuQuyDoiChuaXuat / 1000000, 2)}`;
        
        document.getElementById('rt-kpi-tl-qd-main').innerHTML = `<span class="kpi-percentage">${ui.formatPercentage(phanTramQD)}</span>`;
        ...
        document.getElementById('rt-kpi-dt-tc-sub').innerHTML = `% thực trả chậm: <span class="kpi-percentage">${ui.formatPercentage(phanTramTC)}</span>`;
    },

    // [+] Mới: Hàm render thẻ KPI cho tab Lũy kế
    renderLuykeKpiCards: (luykeData, chuaXuatData, goals) => {
        const targetDTThuc = (parseFloat(goals.doanhThuThuc) || 0) * 1000000000; // tỷ đồng
        const targetDTQD = (parseFloat(goals.doanhThuQD) || 0) * 1000000000; // tỷ đồng
        ...
        document.getElementById('luyke-kpi-dt-thuc-main').textContent = ui.formatNumberOrDash(dtThucLK / 1000000, 2);
        document.getElementById('luyke-kpi-dt-thuc-sub1').innerHTML = `Lũy kế / Target: ${ui.formatNumberOrDash(targetDTThuc / 1000000000, 3)} tỷ`;

        document.getElementById('luyke-kpi-dt-qd-main').textContent = ui.formatNumberOrDash(dtQdLK / 1000000, 2);
        document.getElementById('luyke-kpi-dt-qd-sub1').innerHTML = `% HT: <span class="kpi-percentage">${ui.formatPercentage(phanTramTargetQd)}</span> / Target: ${ui.formatNumberOrDash(targetDTQD / 1000000000, 3)} tỷ`;

        document.getElementById('luyke-kpi-tl-qd-main').innerHTML = `<span class="kpi-percentage">${ui.formatPercentage(phanTramQd)}</span>`;
        document.getElementById('luyke-kpi-tl-qd-sub').textContent = `Mục tiêu: ${ui.formatPercentage((goals.phanTramQD || 0) / 100)}`;
        
        document.getElementById('luyke-kpi-dt-tc-main').textContent = ui.formatNumberOrDash(dtGop / 1000000, 2);
        document.getElementById('luyke-kpi-dt-tc-sub').innerHTML = `% thực trả chậm: <span class="kpi-percentage">${ui.formatPercentage(phanTramGop)}</span>`;

        document.getElementById('luyke-kpi-dt-chuaxuat-qd-main').textContent = ui.formatNumberOrDash(dtChuaXuatQD / 1000000, 2);
    },

    // [*] Sửa đổi: Xóa bỏ hàm renderLuykeCharts
    ...
    renderLuykeCategoryDetailsTable: (data, numDays) => {
        ...
        titleContainer.className = 'text-xl font-bold text-gray-700 mb-4 uppercase header-bg-green p-2 rounded-md flex justify-between items-center';
        if (data.maKho) titleContainer.querySelector('span').textContent = `NGÀNH HÀNG LŨY KẾ - ${data.maKho}`;
        else titleContainer.querySelector('span').textContent = `NGÀNH HÀNG LŨY KẾ`;
    ...
    renderLuykeQdcTable: (data, numDays) => {
        ...
        titleContainer.className = 'text-xl font-bold text-gray-700 mb-4 uppercase header-bg-teal p-2 rounded-md flex justify-between items-center';
         if (data.maKho) titleContainer.querySelector('span').textContent = `NHÓM HÀNG QUY ĐỔI CAO - ${data.maKho}`;
        else titleContainer.querySelector('span').textContent = `NHÓM HÀNG QUY ĐỔI CAO`;
    ...
    renderRealtimeCategoryDetailsTable: (data) => {
        ...
        const titleContainer = document.getElementById('realtime-category-title');
        titleContainer.querySelector('span').textContent = `NGÀNH HÀNG CHI TIẾT`;
    ...
    renderRealtimeQdcTable: (data) => {
        ...
        const titleContainer = document.getElementById('realtime-qdc-title');
        if(!container || !titleContainer) return;
        
        titleContainer.querySelector('span').textContent = 'NHÓM HÀNG QUY ĐỔI CAO';
