...
const ui = {
    ...
    displayHealthKpiTable: (extractedData, goals, updateDailyGoalCallback) => {
        ...
        const dtThucDK = cleanValue(mainKpis['Dự kiến DT thực']) * 1000000;
        const dtQdDK = cleanValue(mainKpis['Dự kiến DTQĐ']) * 1000000;
    ...
    populateAllFilters: () => {
        ...
        ['luyke', 'sknv', 'realtime'].forEach(prefix => {
            const whEl = document.getElementById(`${prefix}-filter-warehouse`);
            const deptEl = document.getElementById(`${prefix}-filter-department`);
            if (whEl) whEl.innerHTML = warehouseOptions;
            if (deptEl) deptEl.innerHTML = departmentOptions;
            
            if (appState.choices[`${prefix}_employee`]) appState.choices[`${prefix}_employee`].destroy();
            if (document.getElementById(`${prefix}-filter-name`)) {
            ...
            if(document.getElementById(`${prefix}-highlight-nhanhang`)){
            ...
            if(document.getElementById(`${prefix}-highlight-nhomhang`)) {
            ...
            if(document.getElementById(`${prefix}-highlight-employee`)){
        ...
        const rtGoalWhEl = document.getElementById('rt-goal-warehouse-select');
        const luykeGoalWhEl = document.getElementById('luyke-goal-warehouse-select');
        if (rtGoalWhEl) rtGoalWhEl.innerHTML = '<option value="">-- Chọn kho để cài đặt --</option>' + warehouses.map(w => `<option value="${w}">${w}</option>`).join('');
        if (luykeGoalWhEl) luykeGoalWhEl.innerHTML = '<option value="">-- Chọn kho để cài đặt --</option>' + warehouses.map(w => `<option value="${w}">${w}</option>`).join('');
    ...
    updateEmployeeFilter: (prefix) => {
        ...
        const sknvFilterEl = document.getElementById('sknv-employee-filter');
        if (prefix === 'sknv' && sknvFilterEl) {
            sknvFilterEl.innerHTML = '<option value="">-- Chọn nhân viên --</option>' + employees.map(nv => `<option value="${String(nv.maNV).trim()}">${nv.hoTen}</option>`).join('');
        }
    },
    ...
    displaySknvReport: (filteredReport) => {
        ...
        const getEvaluation = (value, avgValue) => {
            ...
            if (value >= avgValue) { result.text = 'Trên TB'; result.class = 'text-green-600';} 
        ...
        // ... (phần còn lại của hàm displaySknvReport giữ nguyên)
    },
    ...
    renderRealtimeKpiCards: (data, goals) => {
        ...
        setContent('rt-kpi-dt-thuc-main', ui.formatNumberOrDash(thucHienDTThuc / 1000000, 2));
        ...
        setContent('rt-kpi-dt-qd-main', ui.formatNumberOrDash(thucHienDTQD / 1000000, 2));
        ...
        setContent('rt-kpi-dt-tc-main', ui.formatNumberOrDash((data.doanhThuTraGop || 0) / 1000000, 2));
    ...
    renderLuykeKpiCards: (luykeData, chuaXuatData, goals) => {
        ...
        setContent('luyke-kpi-dt-thuc-main', ui.formatNumberOrDash(dtThucLK / 1000000, 2));
        setContent('luyke-kpi-dt-thuc-sub1', `% HT: <span class="kpi-percentage-value">${ui.formatPercentage(luykeData.phanTramTargetThuc || 0)}</span>`);
        
        setContent('luyke-kpi-dt-qd-main', ui.formatNumberOrDash(dtQdLK / 1000000, 2));
        setContent('luyke-kpi-dt-qd-sub1', `% HT: <span class="kpi-percentage-value">${ui.formatPercentage(phanTramTargetQd)}</span>`);
        ...
        setContent('luyke-kpi-dt-tc-main', ui.formatNumberOrDash(dtGop / 1000000, 2));
        setContent('luyke-kpi-dt-tc-sub', `% thực trả chậm: <span class="kpi-percentage-value">${ui.formatPercentage(phanTramGop)}</span>`);

        setContent('luyke-kpi-dtqd-chua-xuat-main', ui.formatNumberOrDash(dtChuaXuatQD / 1000000, 2));
    },
