...
const app = {
    init() {
        ...
        this.loadInterfaceSettings(); // [+] Mới: Tải cài đặt giao diện
    ...
    // [+] Mới: Tải và áp dụng cài đặt giao diện
    loadInterfaceSettings() {
        try {
            ...
            if (savedSettings) {
                ...
                // Cập nhật giá trị cho các ô input color
                document.getElementById('kpi-card-1-bg').value = settings.kpiCard1Bg || '#38bdf8';
                document.getElementById('kpi-card-2-bg').value = settings.kpiCard2Bg || '#34d399';
                document.getElementById('kpi-card-3-bg').value = settings.kpiCard3Bg || '#fbbf24';
                document.getElementById('kpi-card-4-bg').value = settings.kpiCard4Bg || '#2dd4bf';
                document.getElementById('kpi-card-5-bg').value = settings.kpiCard5Bg || '#a78bfa';
                document.getElementById('kpi-text-color').value = settings.kpiTextColor || '#ffffff';
            }
        } catch (e) {
    ...
    // [+] Mới: Lưu cài đặt giao diện
    saveInterfaceSettings() {
        const settings = {
            kpiCard1Bg: document.getElementById('kpi-card-1-bg').value,
            kpiCard2Bg: document.getElementById('kpi-card-2-bg').value,
            kpiCard3Bg: document.getElementById('kpi-card-3-bg').value,
            kpiCard4Bg: document.getElementById('kpi-card-4-bg').value,
            kpiCard5Bg: document.getElementById('kpi-card-5-bg').value,
    ...
    applyHealthSectionFiltersAndRender() {
        ...
        // [*] Sửa đổi: Hàm này giờ đây render cả thẻ KPI
    ...
    setupEventListeners() {
        ...
        // [+] Mới: Event Listeners cho Drawers
        ...
        // [+] Mới: Event listener cho các ô chọn màu
        ...
        document.querySelectorAll('#employee-subtabs-nav .sub-tab-btn, #realtime-subtabs-nav .sub-tab-btn, #luyke-subtabs-nav .sub-tab-btn').forEach(btn => btn.addEventListener('click', (e) => {
        ...
        document.body.addEventListener('click', (e) => {
            ...
            // [+] Mới: listener cho icon tô màu
            const highlightIcon = e.target.closest('.highlight-icon');
            if(highlightIcon) {
                const section = highlightIcon.closest('.page-section').id.split('-')[0];
                const filterBtn = document.querySelector(`.toggle-filters-btn[data-target="${section}-filter-container"]`);
                const filterContainer = document.getElementById(`${section}-filter-container`);
                if(filterBtn && filterContainer && filterContainer.classList.contains('hidden')) {
                    filterBtn.click();
                }
            }
        });
        
        // [-] Xóa bỏ: Các listener cho toggle-goal-settings-btn cũ
        ...
        document.getElementById('realtime-file-input').addEventListener('change', async (e) => {
            ...
            document.getElementById('realtime-file-input').value = '';
        ...
        const captureDashboard = async (sectionId, chartState, title) => {
            ui.showNotification(`Bắt đầu chụp ảnh ${title}...`, 'success');
            Chart.defaults.animation.duration = 0;
            if(sectionId === 'realtime-section') app.applyRealtimeFiltersAndRender();
            else app.applyHealthSectionFiltersAndRender();
            await new Promise(resolve => setTimeout(resolve, 500));
            const layout = document.createElement('div'); layout.className = 'capture-layout-grid-2x2';
            const panelIds = ['revenue', 'efficiency', 'charts', 'category'];
            const panels = panelIds.map(id => document.getElementById(`${sectionId.split('-')[0]}-${id}-panel`)?.cloneNode(true));
            if (panels.every(p => p)) {
                try {
                    if (chartState.completion) {
                        const img = new Image(); img.src = chartState.completion.toBase64Image();
                        panels[2].querySelector('canvas').parentElement.innerHTML = ''; panels[2].querySelector('canvas').parentElement.appendChild(img);
                    }
                    if (chartState.categoryMix) {
                        const img = new Image(); img.src = chartState.categoryMix.toBase64Image();
                        const canvasContainer = panels[2].querySelectorAll('canvas')[1].parentElement;
                        canvasContainer.innerHTML = ''; canvasContainer.appendChild(img);
                    }
                } catch (e) { console.error("Lỗi chuyển đổi biểu đồ:", e); }
                panels.forEach(p => layout.appendChild(p));
                await captureAndDownload(layout, title);
            } else ui.showNotification('Lỗi: Không tìm thấy đủ các khối để chụp ảnh.', 'error');
            ui.showNotification('Đã hoàn tất chụp ảnh!', 'success');
            Chart.defaults.animation.duration = 1000;
        };

        document.getElementById('capture-employee-health-btn').addEventListener('click', () => {
        ...
        document.getElementById('capture-realtime-btn').addEventListener('click', () => {
            const activeTab = document.querySelector('#realtime-subtabs-nav .sub-tab-btn.active');
            if(activeTab && activeTab.dataset.target === 'subtab-realtime-sieu-thi') {
                const contentToCapture = document.getElementById('subtab-realtime-sieu-thi');
                if (contentToCapture) captureAndDownload(contentToCapture, 'Realtime_SieuThi');
            }
            else {
                const activeTabContent = document.querySelector('#realtime-subtabs-content .sub-tab-content:not(.hidden)');
                if (activeTabContent) captureAndDownload(activeTabContent, `Realtime_${activeTab.textContent.trim()}`);
            }
        });
        document.getElementById('capture-health-btn').addEventListener('click', () => {
            const activeTab = document.querySelector('#luyke-subtabs-nav .sub-tab-btn.active');
            if(activeTab && activeTab.dataset.target === 'subtab-luyke-sieu-thi') {
                 // [*] Sửa đổi: Chụp ảnh không còn phụ thuộc biểu đồ
                const contentToCapture = document.getElementById('subtab-luyke-sieu-thi');
                if (contentToCapture) captureAndDownload(contentToCapture, 'LuyKe_SieuThi');
            } else {
                 const activeTabContent = document.querySelector('#luyke-subtabs-content .sub-tab-content:not(.hidden)');
                 if (activeTabContent) captureAndDownload(activeTabContent, `LuyKe_${activeTab.textContent.trim()}`);
            }
        });
    }
};
