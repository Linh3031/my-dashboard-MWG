...
// Đối tượng app giờ đây gọn gàng hơn, chủ yếu điều phối các hàm từ modules khác.
const app = {
    init() {
    ...
    applyHealthSectionFiltersAndRender() {
        ...
        document.getElementById('luyke-kpi-title').textContent = `DOANH THU LŨY KẾ ${selectedWarehouse ? `- ${selectedWarehouse}` : '- TOÀN CỤM'}`;
    ...
    applyRealtimeFiltersAndRender() {
        ...
        document.getElementById('realtime-kpi-title').textContent = `DOANH THU TẠO REALTIME${titleSuffix}`;
    ...
    setupEventListeners() {
        ...
        document.getElementById('capture-health-btn').addEventListener('click', () => {
            ...
            if(activeTab && activeTab.dataset.target === 'subtab-luyke-sieu-thi') captureDashboard('health-section', { completion: appState.charts.luyke_completion, categoryMix: appState.charts.luyke_categoryMix }, 'LuyKe_SieuThi');
            else {
