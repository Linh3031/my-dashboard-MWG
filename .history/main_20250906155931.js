...
const app = {
    ...
    setupEventListeners() {
        ...
        ['luyke', 'sknv', 'realtime'].forEach(prefix => {
            ...
            if (employeeFilterEl && appState.choices[`${prefix}_employee`]) {
                employeeFilterEl.addEventListener('change', applyFilter);
            }
            ...
            const employeeHighlightEl = document.getElementById(`${prefix}-highlight-employee`);
            if (employeeHighlightEl) {
                employeeHighlightEl.addEventListener('change', () => app.handleHighlightChange(prefix, 'employee'));
            }
        ...
        const captureAndDownload = async (element, title) => {
            ...
            // Check if the title element is ALREADY inside the element to be captured
            const elementIdPrefix = element.id.split('-')[1];
            const titleId = `${elementIdPrefix}-supermarket-title`;
            const titleEl = document.getElementById(titleId);
            
            if (titleEl && !element.contains(titleEl)) {
        ...
        const setupActionButtons = (prefix) => {
            const captureBtnId = prefix === 'luyke' ? 'capture-health-btn' : (prefix === 'sknv' ? 'capture-employee-health-btn' : `capture-${prefix}-btn`);
