...
const app = {
    ...
    loadInterfaceSettings() {
        try {
            ...
            // [*] Sửa lỗi #4: Sử dụng đúng ID từ index.html
            document.getElementById('kpi-color-1').value = settings.kpiCard1Bg || defaultSettings.kpiCard1Bg;
            document.getElementById('kpi-color-2').value = settings.kpiCard2Bg || defaultSettings.kpiCard2Bg;
            document.getElementById('kpi-color-3').value = settings.kpiCard3Bg || defaultSettings.kpiCard3Bg;
            document.getElementById('kpi-color-4').value = settings.kpiCard4Bg || defaultSettings.kpiCard4Bg;
            document.getElementById('kpi-color-5').value = settings.kpiCard5Bg || defaultSettings.kpiCard5Bg;
            document.getElementById('kpi-text-color').value = settings.kpiTextColor || defaultSettings.kpiTextColor;
    ...
    applyHealthSectionFiltersAndRender() {
        ...
        // [*] Sửa lỗi #1: Thêm kiểm tra trước khi dùng .getValue()
    ...
    populateHighlightFilters(prefix, ycxData, reportData) {
        if (!appState.choices[`${prefix}_highlight_nhanhang`]) return; // Guard clause
    ...
    handleHighlightChange(prefix, type) {
        if (!appState.choices[`${prefix}_highlight_${type}`]) return;
    ...
    setupEventListeners() {
        ...
        ['luyke', 'sknv', 'realtime'].forEach(prefix => {
            ...
            document.getElementById(`${prefix}-clear-highlight`).addEventListener('click', () => {
                ...
                ['nhanhang', 'nhomhang', 'employee'].forEach(type => {
                     if (appState.choices[`${prefix}_highlight_${type}`]) {
        ...
        // [*] Sửa lỗi #2: Viết lại logic chuyển tab
        ...
        // [+] Sửa lỗi #5: Thêm logic chuyển tab cho Goal Drawer
        document.querySelectorAll('#goal-drawer-tabs .sub-tab-btn').forEach(btn => btn.addEventListener('click', (e) => {
            const button = e.currentTarget;
            const nav = button.closest('nav');
            const targetId = button.dataset.target;
            const contentContainer = document.getElementById('goal-drawer-content');
