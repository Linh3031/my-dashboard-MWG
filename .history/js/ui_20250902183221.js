...
const ui = {
    ...
    renderRealtimeQdcTable: (data) => {
        ...
        const allQdcKeys = Object.keys(config.PRODUCT_GROUPS.QDC_GROUPS);
        ...
        const tableRows = allQdcKeys.map(key => {
            const itemData = data.qdc ? (data.qdc[key] || { sl: 0, dt: 0, dtqd: 0, name: config.PRODUCT_GROUPS.QDC_GROUPS[key].name }) : { sl: 0, dt: 0, dtqd: 0, name: config.PRODUCT_GROUPS.QDC_GROUPS[key].name };
