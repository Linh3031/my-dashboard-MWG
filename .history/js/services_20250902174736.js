...
const services = {
    ...
    normalizeData(rawData, fileType) {
        ...
        if (!mapping) {
            ...
            return { normalizedData: [], success: false };
        }
        ...
        if (fileType === 'giocong' || fileType === 'thuongnong') {
            if (!foundMapping.maNV && !foundMapping.hoTen) {
                allRequiredFound = false;
                ui.showNotification(`File "${fileType}" phải có cột Mã NV hoặc Tên NV.`, 'error');
            }
        }
        ...
        if (!allRequiredFound) {
            ...
            ui.showNotification(`File "${dataName}" thiếu các cột bắt buộc. Vui lòng kiểm tra công cụ gỡ lỗi.`, 'error');
            return { normalizedData: [], success: false };
        }
