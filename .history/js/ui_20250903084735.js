...
import config from './config.js'; // Thêm import config
...
const ui = {
    ...
    renderLuykeKpiCards: (pastedData, goals) => {
        ...
        // Cập nhật giao diện, loại bỏ sub2
        document.getElementById('luyke-kpi-dt-thuc-main').textContent = ui.formatNumberOrDash(thucHienDTThuc / 1000, 2);
        document.getElementById('luyke-kpi-dt-thuc-sub1').textContent = `% HT: ${ui.formatPercentage(phanTramThucHienThuc)} / Target: ${ui.formatNumberOrDash(targetThuc / 1000, 2)}`;
        document.getElementById('luyke-kpi-dt-thuc-sub2').textContent = `Dự kiến: ${ui.formatNumberOrDash(dtThucDK / 1000, 2)}`;
    
        document.getElementById('luyke-kpi-dt-qd-main').textContent = ui.formatNumberOrDash(thucHienDTQD / 1000, 2);
        document.getElementById('luyke-kpi-dt-qd-sub1').textContent = `% HT: ${ui.formatPercentage(phanTramThucHienQD)} / Target: ${ui.formatNumberOrDash(targetQd / 1000, 2)}`;
        document.getElementById('luyke-kpi-dt-qd-sub2').textContent = `Dự kiến: ${ui.formatNumberOrDash(dtQdDK / 1000, 2)}`;
        ...
        document.getElementById('luyke-kpi-dt-tc-main').textContent = ui.formatNumberOrDash(thucHienDTTc / 1000, 2);
    ...
    renderLuykeQdcTable: (data, numDays) => {
        ...
        // Nhóm lại theo template để hiển thị
        const displayGroups = [
