...
const ui = {
    ...
    displayEmployeeRevenueReport: (reportData, containerId, sortStateKey, departmentGroups) => {
        ...
        departmentGroups.forEach(deptName => {
            if (groupedByDept[deptName]) finalHTML += this.renderRevenueTableForDepartment(deptName, groupedByDept[deptName], sortStateKey);
        });
        finalHTML += `</div>`;
    ...
    renderRevenueTableForDepartment: (title, data, sortStateKey) => {
        ...
        sortedData.forEach(item => {
            ...
            tableHTML += `<tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.doanhThu / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.doanhThuQuyDoi / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold ${qdClass}">${this.formatPercentage(item.hieuQuaQuyDoi)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.doanhThuTraGop / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold ${tcClass}">${this.formatPercentage(item.tyLeTraCham)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.doanhThuChuaXuat / 1000000, 2)}</td></tr>`;
        });
         tableHTML += `</tbody><tfoot class="table-footer font-bold"><tr>
                    <td class="px-4 py-2">Tổng</td>
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.doanhThu / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.doanhThuQuyDoi / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right">${this.formatPercentage(totals.hieuQuaQuyDoi)}</td>
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.doanhThuTraGop / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right">${this.formatPercentage(totals.tyLeTraCham)}</td>
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.doanhThuChuaXuat / 1000000, 2)}</td>
                </tr></tfoot></table></div></div>`;
        return tableHTML;
    },

    displayEmployeeIncomeReport: (reportData, departmentGroups) => {
        ...
        departmentGroups.forEach(deptName => {
            if (groupedByDept[deptName]) finalHTML += this.renderIncomeTableForDepartment(deptName, groupedByDept[deptName]);
        });
        finalHTML += `</div>`;
        container.innerHTML = finalHTML;
    },
    ...
    renderIncomeTableForDepartment: (title, data) => {
        ...
        sortedData.forEach(nv => {
            ...
            tableHTML += `<tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(nv.gioCong, 1)}</td>
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(nv.thuongNong / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right">${this.formatNumberOrDash(nv.thuongERP / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold text-blue-600">${this.formatNumberOrDash(nv.tongThuNhap / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold text-green-600 ${incomeDkCellClass}">${this.formatNumberOrDash(nv.thuNhapDuKien / 1000000, 2)}</td></tr>`;
        });
        tableHTML += `</tbody><tfoot class="table-footer font-bold">
            <tr>
                <td class="px-4 py-2">Tổng</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.gioCong, 1)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.thuongNong / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.thuongERP / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.tongThuNhap / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.thuNhapDuKien / 1000000, 2)}</td>
            </tr>
        </tfoot></table></div></div>`;
        return tableHTML;
    },

    displayEmployeeEfficiencyReport: (reportData, containerId, sortStateKey, departmentGroups) => {
        const container = document.getElementById(containerId);
        if (!container) return;
        if (!reportData || reportData.length === 0) {
        ...
        departmentGroups.forEach(deptName => {
            if (groupedByDept[deptName]) finalHTML += this.renderEfficiencyTableForDepartment(deptName, groupedByDept[deptName], sortStateKey);
        });
        finalHTML += `</div>`;
        container.innerHTML = finalHTML;
    },
    ...
    renderEfficiencyTableForDepartment: (title, data, sortStateKey) => {
        ...
        sortedData.forEach(item => {
            ...
            tableHTML += `<tr class="hover:bg-gray-50">
                <td class="px-4 py-2 font-semibold">${item.hoTen}</td>
                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.dtICT / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.dtPhuKien / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right font-bold ${pkClass}">${this.formatPercentage(item.pctPhuKien)}</td>
                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.dtCE / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item.dtGiaDung / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right font-bold ${gdClass}">${this.formatPercentage(item.pctGiaDung)}</td>
                <td class="px-4 py-2 text-right font-bold ${mlnClass}">${this.formatPercentage(item.pctMLN)}</td>
                <td class="px-4 py-2 text-right font-bold ${simClass}">${this.formatPercentage(item.pctSim)}</td>
                <td class="px-4 py-2 text-right font-bold ${vasClass}">${this.formatPercentage(item.pctVAS)}</td>
                <td class="px-4 py-2 text-right font-bold ${bhClass}">${this.formatPercentage(item.pctBaoHiem)}</td></tr>`;
        });
        tableHTML += `</tbody><tfoot class="table-footer font-bold">
            <tr>
                <td class="px-4 py-2">Tổng</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.dtICT / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.dtPhuKien / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatPercentage(totals.pctPhuKien)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.dtCE / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals.dtGiaDung / 1000000, 2)}</td>
                <td class="px-4 py-2 text-right">${this.formatPercentage(totals.pctGiaDung)}</td>
                <td class="px-4 py-2 text-right">${this.formatPercentage(totals.pctMLN)}</td>
                <td class="px-4 py-2 text-right">${this.formatPercentage(totals.pctSim)}</td>
                <td class="px-4 py-2 text-right">${this.formatPercentage(totals.pctVAS)}</td>
                <td class="px-4 py-2 text-right">${this.formatPercentage(totals.pctBaoHiem)}</td>
            </tr>
        </tfoot></table></div></div>`;
        return tableHTML;
    },
    ...
    displayCategoryRevenueReport: (reportData, containerId, sortStatePrefix) => {
        ...
        let finalHTML = `<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ${this.renderCategoryTable('ICT', `${sortStatePrefix}_ict`, reportData, 'dtICT', 'slICT', ['slDienThoai', 'slLaptop'], ['SL Điện thoại', 'SL Laptop'])}
                ${this.renderCategoryTable('PHỤ KIỆN', `${sortStatePrefix}_phukien`, reportData, 'dtPhuKien', 'slPhuKien', ['slPinSDP', 'slCamera', 'slTaiNgheBLT'], ['SL Pin SDP', 'SL Camera', 'SL Tai nghe BLT'])}
                ${this.renderCategoryTable('GIA DỤNG', `${sortStatePrefix}_giadung`, reportData, 'dtGiaDung', 'slGiaDung', ['slNoiChien', 'slMLN', 'slRobotHB'], ['SL Nồi chiên', 'SL MLN', 'SL Robot HB'])}
                ${this.renderCategoryTable('CE', `${sortStatePrefix}_ce`, reportData, 'dtCE', 'slCE', ['slTivi', 'slTuLanh', 'slMayGiat', 'slMayLanh'], ['SL Tivi', 'SL Tủ lạnh', 'SL Máy giặt', 'SL Máy lạnh'])}
                <div class="lg:col-span-2">
                    ${this.renderCategoryTable('BẢO HIỂM', `${sortStatePrefix}_baohiem`, reportData, 'dtBaoHiem', 'slBaoHiem', ['slBH1d1', 'slBHXM', 'slBHRV', 'slBHMR'], ['BH 1-1', 'BHXM', 'BHRV', 'BHMR'])}
                </div></div>`;
        container.innerHTML = finalHTML;
    },
    ...
    renderCategoryTable: (title, type, data, revenueField, slField, keys, headers) => {
        ...
        sortedData.forEach(item => {
            tableHTML += `<tr class="hover:bg-gray-50"><td class="px-4 py-2 font-semibold">${item.hoTen}</td>
                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item[slField])}</td>
                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item[revenueField] / 1000000, 2)}</td>`;
            keys.forEach(k => { tableHTML += `<td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(item[k])}</td>`; });
            tableHTML += `</tr>`;
        });
        tableHTML += `</tbody><tfoot class="table-footer font-bold">
                <tr><td class="px-4 py-2">Tổng</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals[slField])}</td>
                <td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals[revenueField] / 1000000, 2)}</td>`;
        keys.forEach(k => { tableHTML += `<td class="px-4 py-2 text-right">${this.formatNumberOrDash(totals[k])}</td>`; });
    ...
    displaySknvReport: (filteredReport, allDepartmentAverages) => {
        ...
        if (!selectedMaNV) {
            ...
            this.displaySknvSummaryReport(filteredReport, allDepartmentAverages);
            return;
        }
        ...
        const createDoanhThuChiTietTableHtml = (title, colorClass, data) => {
            const rows = Object.entries(data).sort(([, a], [, b]) => b.revenue - a.revenue).map(([nganhHang, values]) => `
                    <tr class="border-t"><td class="px-4 py-2 font-medium text-gray-700">${nganhHang}</td><td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(values.quantity)}</td><td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(values.revenue / 1000000, 2)}</td></tr>`).join('');
            if (!rows) return '';
            return `<div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden"><h4 class="text-lg font-bold p-3 border-b ${colorClass}">${title}</h4><table class="min-w-full text-sm table-bordered table-striped">
        ...
        const createQdcTableHtml = (title, colorClass, employeeData, departmentAverages) => {
            let tableContent = '';
            const qdcGroups = config.PRODUCT_GROUPS.QDC_GROUPS;
            
            for (const key in qdcGroups) {
                const empQdcData = employeeData.qdc[key];
                if (empQdcData && empQdcData.sl > 0) {
                    const avgQdcData = departmentAverages.qdc ? (departmentAverages.qdc[key] || { dtqd: 0 }) : { dtqd: 0 };
                    const evaluation = getEvaluation(empQdcData.dtqd, avgQdcData.dtqd);
                    tableContent += `<tr class="border-t"><td class="px-4 py-2 font-medium text-gray-700">${empQdcData.name}</td><td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(empQdcData.sl)}</td><td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(empQdcData.dt / 1000000, 2)}</td><td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(empQdcData.dtqd / 1000000, 2)}</td><td class="px-4 py-2 text-center font-semibold ${evaluation.class}">${evaluation.text}</td></tr>`;
                }
            }
        ...
        const doanhThuData = [
            ...
            { label: 'Doanh thu CE', value: this.formatNumberOrDash(employeeData.dtCE / 1000000, 2), average: this.formatNumberOrDash((departmentAverages.dtCE || 0) / 1000000, 2), rawValue: employeeData.dtCE, rawAverage: departmentAverages.dtCE },
            { label: 'Doanh thu ICT', value: this.formatNumberOrDash(employeeData.dtICT / 1000000, 2), average: this.formatNumberOrDash((departmentAverages.dtICT || 0) / 1000000, 2), rawValue: employeeData.dtICT, rawAverage: departmentAverages.dtICT },
            { label: 'Doanh thu trả chậm', value: this.formatNumberOrDash(employeeData.doanhThuTraGop / 1000000, 2), average: this.formatNumberOrDash((departmentAverages.doanhThuTraGop || 0) / 1000000, 2), rawValue: employeeData.doanhThuTraGop, rawAverage: departmentAverages.doanhThuTraGop },
        ...
        const nangSuatData = [
            ...
            { label: 'Thu nhập lũy kế', value: this.formatNumberOrDash(employeeData.tongThuNhap / 1000000, 2), average: this.formatNumberOrDash((departmentAverages.tongThuNhap || 0) / 1000000, 2), rawValue: employeeData.tongThuNhap, rawAverage: departmentAverages.tongThuNhap },
            ...
            { label: 'Giờ công', value: this.formatNumberOrDash(employeeData.gioCong, 1), average: this.formatNumberOrDash(departmentAverages.gioCong, 1), rawValue: employeeData.gioCong, rawAverage: departmentAverages.gioCong },
            { label: 'Thu nhập/GC', value: this.formatNumberOrDash(employeeData.gioCong > 0 ? employeeData.tongThuNhap / employeeData.gioCong : 0, 0), average: this.formatNumberOrDash((departmentAverages.gioCong || 0) > 0 ? (departmentAverages.tongThuNhap || 0) / departmentAverages.gioCong : 0, 0), rawValue: employeeData.gioCong > 0 ? employeeData.tongThuNhap / employeeData.gioCong : 0, rawAverage: (departmentAverages.gioCong || 0) > 0 ? (departmentAverages.tongThuNhap || 0) / departmentAverages.gioCong : 0 },
        ...
        const hieuQuaData = [
            { label: '% PK', value: this.formatPercentage(employeeData.pctPhuKien), valueClass: employeeData.pctPhuKien < (mucTieu.phanTramPhuKien/100) ? 'cell-performance is-below' : '', average: this.formatPercentage(departmentAverages.pctPhuKien), rawValue: employeeData.pctPhuKien, rawAverage: departmentAverages.pctPhuKien },
            { label: '% Gia dụng', value: this.formatPercentage(employeeData.pctGiaDung), valueClass: employeeData.pctGiaDung < (mucTieu.phanTramGiaDung/100) ? 'cell-performance is-below' : '', average: this.formatPercentage(departmentAverages.pctGiaDung), rawValue: employeeData.pctGiaDung, rawAverage: departmentAverages.pctGiaDung },
            { label: '% MLN', value: this.formatPercentage(employeeData.pctMLN), valueClass: employeeData.pctMLN < (mucTieu.phanTramMLN/100) ? 'cell-performance is-below' : '', average: this.formatPercentage(departmentAverages.pctMLN), rawValue: employeeData.pctMLN, rawAverage: departmentAverages.pctMLN },
            { label: '% Sim', value: this.formatPercentage(employeeData.pctSim), valueClass: employeeData.pctSim < (mucTieu.phanTramSim/100) ? 'cell-performance is-below' : '', average: this.formatPercentage(departmentAverages.pctSim), rawValue: employeeData.pctSim, rawAverage: departmentAverages.pctSim },
            { label: '% VAS', value: this.formatPercentage(employeeData.pctVAS), valueClass: employeeData.pctVAS < (mucTieu.phanTramVAS/100) ? 'cell-performance is-below' : '', average: this.formatPercentage(departmentAverages.pctVAS), rawValue: employeeData.pctVAS, rawAverage: departmentAverages.pctVAS },
            { label: '% Bảo hiểm', value: this.formatPercentage(employeeData.pctBaoHiem), valueClass: employeeData.pctBaoHiem < (mucTieu.phanTramBaoHiem/100) ? 'cell-performance is-below' : '', average: this.formatPercentage(departmentAverages.pctBaoHiem), rawValue: employeeData.pctBaoHiem, rawAverage: departmentAverages.pctBaoHiem },
        ];
        ...
        const qdcHtml = createQdcTableHtml('Nhóm hàng quy đổi cao', 'header-bg-teal', employeeData, departmentAverages);
    ...
    displaySknvSummaryReport: (reportData, allDepartmentAverages) => {
        ...
        const summaryData = reportData.map(employee => {
            const departmentAverages = allDepartmentAverages[employee.boPhan] || {};
            const counts = {
        ...
        const departmentGroups = [...new Set(summaryData.map(item => item.boPhan))];
    ...
    renderLuykeKpiCards: (pastedData, goals) => {
        const { mainKpis } = pastedData;
        ...
        const thucHienDTThuc = cleanValue(mainKpis['Thực hiện DT thực']);
        const thucHienDTQD = cleanValue(mainKpis['Thực hiện DTQĐ']);
        const dtThucDK = cleanValue(mainKpis['Dự kiến DT thực']);
        const dtQdDK = cleanValue(mainKpis['Dự kiến DTQĐ']);
        const thucHienDTTc = cleanValue(mainKpis['Doanh thu trả chậm']);
    
        const phanTramThucHienThuc = targetThuc > 0 ? dtThucDK / targetThuc : 0;
        const phanTramThucHienQD = targetQd > 0 ? dtQdDK / targetQd : 0;
    
        const phanTramQD = thucHienDTThuc > 0 ? (thucHienDTQD / thucHienDTThuc) - 1 : 0;
        const targetPTQD = (parseFloat(goals.phanTramQD) || 0) / 100;
    
        const phanTramTC = thucHienDTThuc > 0 ? thucHienDTTc / thucHienDTThuc : (cleanValue(mainKpis['% Trả chậm']) / 100);
    
        document.getElementById('luyke-kpi-dt-thuc-main').textContent = this.formatNumberOrDash(thucHienDTThuc / 1000, 2);
        document.getElementById('luyke-kpi-dt-thuc-sub1').textContent = `% HT: ${this.formatPercentage(phanTramThucHienThuc)} / Target: ${this.formatNumberOrDash(targetThuc / 1000, 2)}`;
        document.getElementById('luyke-kpi-dt-thuc-sub2').textContent = `Dự kiến: ${this.formatNumberOrDash(dtThucDK / 1000, 2)}`;
    
        document.getElementById('luyke-kpi-dt-qd-main').textContent = this.formatNumberOrDash(thucHienDTQD / 1000, 2);
        document.getElementById('luyke-kpi-dt-qd-sub1').textContent = `% HT: ${this.formatPercentage(phanTramThucHienQD)} / Target: ${this.formatNumberOrDash(targetQd / 1000, 2)}`;
        document.getElementById('luyke-kpi-dt-qd-sub2').textContent = `Dự kiến: ${this.formatNumberOrDash(dtQdDK / 1000, 2)}`;
        
        document.getElementById('luyke-kpi-tl-qd-main').textContent = this.formatPercentage(phanTramQD);
        document.getElementById('luyke-kpi-tl-qd-sub').textContent = `Mục tiêu: ${this.formatPercentage(targetPTQD)}`;
    
        document.getElementById('luyke-kpi-dt-tc-main').textContent = this.formatNumberOrDash(thucHienDTTc / 1000, 2);
        document.getElementById('luyke-kpi-dt-tc-sub').textContent = `% thực trả chậm: ${this.formatPercentage(phanTramTC)}`;
    },
    ...
    renderRealtimeKpiCards: (data, goals) => {
        ...
        document.getElementById('rt-kpi-dt-thuc-main').textContent = this.formatNumberOrDash(thucHienDTThuc / 1000000, 2);
        document.getElementById('rt-kpi-dt-thuc-sub1').textContent = `% HT: ${this.formatPercentage(phanTramThucHienThuc)} / Target: ${this.formatNumberOrDash(targetDTThuc / 1000000, 2)}`;
        document.getElementById('rt-kpi-dt-thuc-sub2').textContent = `DT Giao xa: ${this.formatNumberOrDash(data.doanhThuGiaoXa / 1000000, 2)}`;
    
        document.getElementById('rt-kpi-dt-qd-main').textContent = this.formatNumberOrDash(thucHienDTQD / 1000000, 2);
        document.getElementById('rt-kpi-dt-qd-sub1').textContent = `% HT: ${this.formatPercentage(phanTramThucHienQD)} / Target: ${this.formatNumberOrDash(targetDTQD / 1000000, 2)}`;
        document.getElementById('rt-kpi-dt-qd-sub2').textContent = `DT Giao xa: ${this.formatNumberOrDash(data.doanhThuQuyDoiGiaoXa / 1000000, 2)}`;
        
        document.getElementById('rt-kpi-tl-qd-main').textContent = this.formatPercentage(phanTramQD);
        document.getElementById('rt-kpi-tl-qd-sub').textContent = `Mục tiêu: ${this.formatPercentage(targetPTQD)}`;
    
        document.getElementById('rt-kpi-dt-tc-main').textContent = this.formatNumberOrDash(data.doanhThuTraGop / 1000000, 2);
        document.getElementById('rt-kpi-dt-tc-sub').textContent = `% thực trả chậm: ${this.formatPercentage(phanTramTC)}`;
    },
                    ...
                    <thead class="text-xs text-slate-800 uppercase bg-slate-200 font-bold">
                        <tr>
                            ...
                            <tr class="hover:bg-gray-50"><td class="px-4 py-2 font-semibold">${cat.name}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(cat.quantity)}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(cat.revenue / 1000000, 2)}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(numDays > 0 ? cat.quantity / numDays : 0, 1)}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(numDays > 0 ? (cat.revenue / numDays) / 1000000 : 0, 2)}</td>
    ...
    renderLuykeQdcTable: (data, numDays) => {
        ...
        const displayGroups = [
            { title: 'Nhóm ICT', keys: ['PIN_SDP', 'TAI_NGHE_BLT', 'DONG_HO', 'CAMERA', 'LOA'], colorClass: 'qdc-group-ict' },
            { title: 'VAS', keys: ['UDDD', 'BAO_HIEM'], colorClass: 'qdc-group-vas' },
            { title: 'Gia dụng', keys: ['NOI_COM', 'NOI_CHIEN', 'MAY_LOC_NUOC', 'ROBOT_HB'], colorClass: 'qdc-group-giadung' },
            { title: 'SIM', keys: ['SIM_ONLINE'], colorClass: 'qdc-group-sim' }
        ];

        displayGroups.forEach((group) => {
            const groupRows = group.keys.map(itemKey => {
                ...
                if (itemData && itemData.sl > 0) return `<tr class="border-t"><td class="px-4 py-2 font-medium text-gray-700">${itemData.name}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(itemData.sl)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(itemData.dtqd / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(numDays > 0 ? itemData.sl / numDays : 0, 1)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(numDays > 0 ? (itemData.dtqd / numDays) / 1000000 : 0, 2)}</td></tr>`;
                return '';
            }).join('');
        ...
        container.innerHTML = `<div class="overflow-x-auto"><table class="min-w-full text-sm text-left text-gray-600 table-bordered table-striped">
                    <thead class="text-xs text-slate-800 uppercase bg-slate-200 font-bold">
                        <tr><th class="px-4 py-2">Tiêu chí</th><th class="px-4 py-2 text-right">Thực hiện</th><th class="px-4 py-2 text-right">Mục tiêu</th></tr></thead><tbody>${rows.map(row => 
                            ...
                            <td class="px-4 py-2 text-right font-bold ${row.mucTieu > 0 && row.thucHien < row.mucTieu ? 'cell-performance is-below' : ''}">${this.formatPercentage(row.thucHien)}</td>
                            <td class="px-4 py-2 text-right font-bold">${this.formatPercentage(row.mucTieu)}</td></tr>`).join('')}</tbody></table></div>`;
    },
                    ...
                    <thead class="text-xs text-slate-800 uppercase bg-slate-200 font-bold">
                        <tr>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(cat.quantity)}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(cat.revenue / 1000000, 2)}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(cat.revenueQuyDoi / 1000000, 2)}</td>
                                <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(cat.donGia / 1000000, 2)}</td></tr>`).join('')}</tbody></table></div>`;
    },
    ...
    renderRealtimeEfficiencyTable: (data, goals) => {
        ...
        container.innerHTML = `<div class="overflow-x-auto"><table class="min-w-full text-sm text-left text-gray-600 table-bordered table-striped">
                    <thead class="text-xs text-slate-800 uppercase bg-slate-200 font-bold">
                        <tr><th class="px-4 py-2">Tiêu chí</th><th class="px-4 py-2 text-right">Thực hiện</th><th class="px-4 py-2 text-right">Mục tiêu</th></tr></thead><tbody>${rows.map(row => 
                            `<tr class="hover:bg-gray-50"><td class="px-4 py-2 font-semibold">${row.label}</td>
                            <td class="px-4 py-2 text-right font-bold ${row.thucHien < row.mucTieu ? 'cell-performance is-below' : ''}">${this.formatPercentage(row.thucHien)}</td>
                            <td class="px-4 py-2 text-right font-bold">${this.formatPercentage(row.mucTieu)}</td></tr>`).join('')}</tbody></table></div>`;
    },
    ...
    renderRealtimeQdcTable: (data) => {
        ...
        const qdcGroups = config.PRODUCT_GROUPS.QDC_GROUPS;
        const allQdcKeys = Object.keys(qdcGroups);
    
        if(allQdcKeys.length === 0) {
             container.innerHTML = `<p class="text-gray-500 font-bold">Không có dữ liệu.</p>`; return;
        }
        
        const tableRows = allQdcKeys.map(key => {
            const itemData = data.qdc[key] || { sl: 0, dt: 0, dtqd: 0, name: qdcGroups[key].name };
            const donGia = itemData.sl > 0 ? itemData.dt / itemData.sl : 0;
            return `
                <tr class="border-t">
                    <td class="px-4 py-2 font-medium text-gray-700">${itemData.name}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(itemData.sl)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(itemData.dt / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(itemData.dtqd / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold">${this.formatNumberOrDash(donGia / 1000000, 2)}</td>
                </tr>
            `;
        }).join('');
