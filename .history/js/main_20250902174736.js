    ...
    const app = {
        ...
        async handleFileRead(file, fileType) {
             ...
             try {
                 ...
                 if (success) {
                     appState[`${fileType}Data`] = normalizedData; // Gán dữ liệu vào đúng state
                     ...
                     if (fileStatusSpan) {
                         fileStatusSpan.textContent = `✓ Đã tải ${normalizedData.length} dòng.`;
                     ...
                     if (fileStatusSpan) {
                         fileStatusSpan.textContent = `Lỗi: Thiếu cột dữ liệu.`;
        ...
        setupEventListeners() {
            ...
            document.getElementById('realtime-file-input').addEventListener('change', async (e) => {
                ...
                try {
                    const rawData = await app.handleFileRead(file, 'ycx');
                    // Logic xử lý file realtime giờ nằm trong handleFileRead
