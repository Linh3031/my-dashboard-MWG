...
const services = {
    ...
    normalizeData(rawData, fileType) {
        ...
        if (!mapping) {
            ...
            return { normalizedData: [], success: false, message: `Không tìm thấy cấu hình cho loại file: ${fileType}` };
        }
        ...
        if (fileType === 'giocong' || fileType === 'thuongnong') {
            if (!foundMapping.maNV && !foundMapping.hoTen) {
                allRequiredFound = false;
                 return { normalizedData: [], success: false, message: `File "${fileType}" phải có cột Mã NV hoặc Tên NV.` };
            }
        }

        if (!allRequiredFound) {
            const dataName = document.querySelector(`#file-${fileType}`)?.dataset.name || fileType;
            return { normalizedData: [], success: false, message: `File "${dataName}" thiếu các cột bắt buộc. Vui lòng kiểm tra công cụ gỡ lỗi.` };
        }
    ...
    parseLuyKePastedData: (text) => {
        ...
        const patterns = {
            ...
            '% Hoàn thành DT QĐ': /% HT Target Dự Kiến \(QĐ\)\s+([\d.]+%?)/,
    ...
    generateMasterReportData: (sourceData, goalSettings, isRealtime = false) => {
        ...
        return appState.danhSachNhanVien.map((employee) => {
            let data = {
                doanhThu: 0, doanhThuQuyDoi: 0, doanhThuTraGop: 0, doanhThuTraGopQuyDoi: 0, doanhThuChuaXuat: 0,
            ...
            sourceData.forEach(row => {
                ...
                if (msnvMatch && msnvMatch[1].trim() === employee.maNV) {
                    ...
                    if (isBaseValid && isDoanhThuHTX) {
                        ...
                        if ((row.trangThaiXuat || "").trim() === 'Chưa xuất') data.doanhThuChuaXuat += thanhTien;
                        ...
                        if (row.hinhThucXuat !== 'Xuất dịch vụ thu hộ bảo hiểm') { 
                            ...
                            if (row.ngayTao && row.ngayHenGiao) {
