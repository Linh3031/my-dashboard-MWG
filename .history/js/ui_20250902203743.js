...
const ui = {
    ...
    displaySknvReport: (filteredReport, allDepartmentAverages, qdcGroups) => {
        ...
        if (!selectedMaNV) {
            ...
            ui.displaySknvSummaryReport(filteredReport, allDepartmentAverages);
            return;
        }
        ...
        const createQdcTableHtml = (title, colorClass, qdcGroups, employeeData, departmentAverages) => {
            ...
            Object.keys(qdcGroups).forEach(groupKey => {
                let groupHasData = false;
                const groupItems = qdcGroups[groupKey].codes;
        
                const groupRows = groupItems.map(itemKey => {
                ...
                if (groupHasData) {
                    tableContent += `<tr class="qdc-group-title ${qdcGroups[groupKey].colorClass || ''}"><td colspan="5" class="px-4 py-2">${qdcGroups[groupKey].name}</td></tr>` + groupRows;
                }
            });
        ...
        // Render all tables to calculate evaluationCounts
        ...
        const qdcHtml = createQdcTableHtml('Nhóm hàng quy đổi cao', 'header-bg-teal', qdcGroups, employeeData, departmentAverages);
    ...
    renderLuykeKpiCards: (pastedData, goals, aggregatedLuykeData) => {
        ...
        const targetQd = (parseFloat(goals.doanhThuQD) || 0) * 1000000000;
    
        const dtThucDK = cleanValue(pastedData.mainKpis['Dự kiến DT thực']);
        const dtQdDK = cleanValue(pastedData.mainKpis['Dự kiến DTQĐ']);
    
        const thucHienDTThuc = aggregatedLuykeData.doanhThu;
        const thucHienDTQD = aggregatedLuykeData.doanhThuQuyDoi;
        ...
        const thucHienDTTc = aggregatedLuykeData.doanhThuTraGop;
        const phanTramTC = thucHienDTThuc > 0 ? thucHienDTTc / thucHienDTThuc : 0;
    
        document.getElementById('luyke-kpi-dt-thuc-main').textContent = ui.formatNumberOrDash(thucHienDTThuc / 1000000, 2);
        document.getElementById('luyke-kpi-dt-thuc-sub1').textContent = `% HT: ${ui.formatPercentage(phanTramThucHienThuc)} / Target: ${ui.formatNumberOrDash(targetThuc / 1000000000, 2)}`;
        document.getElementById('luyke-kpi-dt-thuc-sub2').textContent = `DT Chưa xuất: ${ui.formatNumberOrDash(aggregatedLuykeData.doanhThuChuaXuat / 1000000, 2)}`;
    
        document.getElementById('luyke-kpi-dt-qd-main').textContent = ui.formatNumberOrDash(thucHienDTQD / 1000000, 2);
        document.getElementById('luyke-kpi-dt-qd-sub1').textContent = `% HT: ${ui.formatPercentage(phanTramThucHienQD)} / Target: ${ui.formatNumberOrDash(targetQd / 1000000000, 2)}`;
        document.getElementById('luyke-kpi-dt-qd-sub2').textContent = `DT Giao xa: ${ui.formatNumberOrDash(aggregatedLuykeData.doanhThuQuyDoiGiaoXa / 1000000, 2)}`;
        
        document.getElementById('luyke-kpi-tl-qd-main').textContent = ui.formatPercentage(phanTramQD);
        document.getElementById('luyke-kpi-tl-qd-sub').textContent = `Mục tiêu: ${ui.formatPercentage(targetPTQD)}`;
    
        document.getElementById('luyke-kpi-dt-tc-main').textContent = ui.formatNumberOrDash(thucHienDTTc / 1000000, 2);
        document.getElementById('luyke-kpi-dt-tc-sub').textContent = `% thực trả chậm: ${ui.formatPercentage(phanTramTC)}`;
    },
    ...
    renderLuykeQdcTable: (data, numDays, qdcGroups) => {
        ...
        Object.keys(qdcGroups).forEach(groupKey => {
            const group = qdcGroups[groupKey];
            let groupHasData = false;
            
            const groupRows = group.codes.map(itemCode => {
                const itemData = Object.values(data.qdc).find(d => d.code === itemCode); // Assuming 'code' property exists
                if (itemData && itemData.sl > 0) {
                    groupHasData = true;
                    return `<tr class="border-t"><td class="px-4 py-2 font-medium text-gray-700">${itemData.name}</td>
            ...
            if (groupHasData) {
                tableContent += `<tr class="qdc-group-title ${group.colorClass || ''}"><td colspan="5" class="px-4 py-2">${group.name}</td></tr>` + groupRows;
            }
        });
    ...
    renderRealtimeQdcTable: (data, qdcGroups) => {
        ...
        const tableRows = allQdcKeys.map(key => {
            const itemData = data.qdc ? (data.qdc[key] || { sl: 0, dt: 0, dtqd: 0, name: qdcGroups[key].name }) : { sl: 0, dt: 0, dtqd: 0, name: qdcGroups[key].name };
            const donGia = itemData.sl > 0 ? itemData.dt / itemData.sl : 0;
            return `
                <tr class="border-t">
                    <td class="px-4 py-2 font-medium text-gray-700">${itemData.name}</td>
                    <td class="px-4 py-2 text-right font-bold">${ui.formatNumberOrDash(itemData.sl)}</td>
                    <td class="px-4 py-2 text-right font-bold">${ui.formatNumberOrDash(itemData.dt / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold">${ui.formatNumberOrDash(itemData.dtqd / 1000000, 2)}</td>
                    <td class="px-4 py-2 text-right font-bold">${ui.formatNumberOrDash(donGia / 1000000, 2)}</td>
                </tr>
            `;
        }).join('');
