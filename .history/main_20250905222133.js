...
const app = {
    ...
    exportTableToExcel(activeTabContent, fileName) {
        const table = activeTabContent.querySelector('table:not(.choices__table)');
        if (!table) {
            ui.showNotification('Không tìm thấy bảng để xuất.', 'error');
            return;
        }
    ...
    setupEventListeners() {
        ...
        const captureAndDownload = async (element, title) => {
            ...
            // Add title element for capture
            const titleEl = element.closest('.sub-tab-content')?.previousElementSibling;
            if(titleEl && titleEl.tagName === 'H2') {
                captureContainer.appendChild(titleEl.cloneNode(true));
            }
            captureContainer.appendChild(clone);
            document.body.appendChild(captureContainer);
            try {
                const canvas = await html2canvas(captureContainer, { scale: 2, useCORS: true, backgroundColor: '#f3f4f6' });
